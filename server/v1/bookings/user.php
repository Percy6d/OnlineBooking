...
if(!empty($getData)){
    ...
    if(isset($getData->userID)){
        $getUserBookings = $booking->getUserBookings($getData->userID);
        if(is_array($getUserBookings) || is_object($getUserBookings)){
            echo json_encode($getUserBookings);
        } else {
            echo $getUserBookings;
        }
    } else {
